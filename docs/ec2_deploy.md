...
For legacy API/proxy accounts, keep `CLOB_SIGNATURE_TYPE=POLY_PROXY` and set
...
`CLOB_SIGNATURE_TYPE=2`; set `CLOB_FUNDER_ADDRESS` to the same address
previously used as `FUNDER_ADDRESS`.
