...
Binance USD-M Futures aggTrade ─► observational audit
                                  │                    │
...
`src/aftertake/twap_tail.py` is pure decision logic. It accepts timestamped CLOB quotes and optional Futures
observations, makes the entry decision from PM only, and returns ENTER/HOLD plus audit fields. It has no HTTP,
...
disconnect/reconnect during the candle, missing tape, or buffer overflow is retained for audit only.

`src/aftertake/runner.py` owns schedule, Gamma metadata gate, streams, live account checks, risk, SQLite
reservation, order submission and audit. It may join an active round when enough preflight lead remains.
