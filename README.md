...
配合 Binance Spot 的連續價格路徑做一次因果決策。

Binance 只是尾段反轉的 path filter；最終結算仍由 Polymarket 官方結果決定。Spot/CLOB 資料不完整、
...
`.env.example` 預設 `AFTERTAKE_DRY_RUN=true`：會連 PM CLOB 與 Binance Spot、寫入 timing/hold
證據，但不簽名或送單。程式在啟動或重連後會等下一個完整的五分鐘 boundary，避免用不完整的 Spot
candle。
